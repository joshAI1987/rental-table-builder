...
class RentalDataAnalyzer:
    ...
    def collect_data(self):
        ...
        # Normalize the strings before comparison to make it more robust
        selected_name_normalized = self.selected_geo_name.lower().replace("(", "").replace(")", "").strip()
        greater_sydney_lgas_normalized = [lga.lower().replace("(", "").replace(")", "").strip() 
                                     for lga in self.GREATER_SYDNEY_LGAS]
        is_greater_sydney = selected_name_normalized in greater_sydney_lgas_normalized
                        ...
                        if not df_filtered.empty:
                            ...
                            if rate_col and len(df_latest) > 0:
                                ...
                                if change_col and len(df_latest) > 0:
                                    annual_change_value = df_latest[change_col].iloc[0]
