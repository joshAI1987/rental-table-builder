...
# Input for root folder path with clearer instructions
st.sidebar.markdown("### Enter the path to your data folder:")
st.sidebar.markdown("Enter the full path to the folder containing your rental data files.")
st.sidebar.markdown("Example: `C:\\Users\\username\\Documents\\RentalData`")

root_folder = st.sidebar.text_input("Folder path:", ".")

# Option to upload files directly
st.sidebar.markdown("### Or upload files directly:")
st.sidebar.markdown("You can upload Excel or Parquet files directly.")

uploaded_files = st.sidebar.file_uploader("Upload data files", 
                                         type=["xlsx", "xls", "parquet"], 
                                         accept_multiple_files=True)

# Save uploaded files to temp directory if any are uploaded
if uploaded_files:
    import os
    
    # Create temp directory if it doesn't exist
    if not hasattr(analyzer, 'temp_dir'):
        analyzer.temp_dir = tempfile.mkdtemp()
    
    # Save uploaded files to temp directory
    for uploaded_file in uploaded_files:
        file_path = os.path.join(analyzer.temp_dir, uploaded_file.name)
        with open(file_path, "wb") as f:
            f.write(uploaded_file.getbuffer())
    
    # Set root folder to temp directory
    root_folder = analyzer.temp_dir
    
    st.sidebar.success(f"{len(uploaded_files)} files uploaded!")

# Add a button to scan the root folder with better error handling
if st.sidebar.button("Scan Root Folder"):
    if os.path.exists(root_folder):
        ...
        if sum(len(files) for files in files_dict.values()) > 0:
            ...
            st.warning("No data files found. Please check the root folder path or upload files directly.")
    else:
        st.error(f"Root folder '{root_folder}' does not exist. Please enter a valid path.")
        st.markdown("""
        **Troubleshooting tips:**
        - Make sure the path is complete and correct
        - Use double backslashes in Windows paths: `C:\\Users\\username\\Documents`
        - Or use forward slashes: `C:/Users/username/Documents`
        - Or try uploading files directly using the file uploader above
        """)
            ...
            if st.sidebar.button("Generate Analysis"):
                ...
                if st.button("Generate Excel Report"):
                    ...
                    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                        ...
                        if data.get("median_rent", {}).get("time_series"):
                            df_rent = pd.DataFrame(data["median_rent"]["time_series"])
                            df_rent.to_excel(writer, sheet_name="Median Rent Time Series", index=False)
                        
                        if data.get("vacancy_rates", {}).get("time_series"):
                            df_vacancy = pd.DataFrame(data["vacancy_rates"]["time_series"])
                            df_vacancy.to_excel(writer, sheet_name="Vacancy Rates Time Series", index=False)
                        
                        if data.get("affordability", {}).get("time_series"):
                            df_afford = pd.DataFrame(data["affordability"]["time_series"])
                            df_afford.to_excel(writer, sheet_name="Affordability Time Series", index=False)
                    
                    # Offer the Excel file for download
                    excel_data = excel_buffer.getvalue()
                    b64 = base64.b64encode(excel_data).decode()
                    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{selected_geo_name}_rental_analysis.xlsx">Download Excel Report</a>'
                    st.markdown(href, unsafe_allow_html=True)
        else:
            st.warning(f"No geographic names found for {selected_geo_area}. Try selecting a different geographic area type.")
else:
    st.info("Please scan a root folder or upload files to load data.")
