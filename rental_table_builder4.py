...
class RentalDataAnalyzer:
    ...
    def scan_root_folder(self, root_folder):
        ...
        self.uploaded_files = files_dict
        return files_dict
    ...
    def collect_data(self, uploaded_files):
        """Collect data for the selected geographic area and name"""
        self.data = {}
        
        # Collect data from various sources
        with st.spinner(f"Collecting data for {self.selected_geo_name} ({self.selected_geo_area})..."):
    ...
    def collect_census_data(self, uploaded_files):
        ...
        try:
            ...
            for file_data in uploaded_files.get("census_dwelling", []):
                if file_data['geo_area'].lower() == self.selected_geo_area.lower():
                    file_path = file_data['path']
                    df = self.read_data_file(file_path)
    ...
    def display_dashboard(self):
        ...
        # Header
        st.header(f"Rental Analysis for {self.selected_geo_name}")
        ...
        with col1:
            ...
            renters_data = self.data["renters"]
            st.metric(
            ...
            social_data = self.data["social_housing"]
            st.metric(
        ...
        with col2:
            ...
            rent_data = self.data["median_rent"]
            st.metric(
            ...
            vacancy_data = self.data["vacancy_rates"]
        ...
        with col3:
            ...
            afford_data = self.data["affordability"]
        ...
        st.info(self.generate_comparison_comment("renters", self.data['renters']['percentage'], 
                                    self.data['renters']['comparison_gs'], self.data['renters']['comparison_ron']))
        
        st.info(self.generate_comparison_comment("social_housing", self.data['social_housing']['percentage'], 
                                    self.data['social_housing']['comparison_gs'], self.data['social_housing']['comparison_ron']))
        
        st.info(self.generate_comparison_comment("median_rent", self.data['median_rent']['value'], 
                                   self.data['median_rent']['comparison_gs'], self.data['median_rent']['comparison_ron']))
        
        st.info(self.generate_comparison_comment("vacancy_rates", self.data['vacancy_rates']['value'], 
                                    self.data['vacancy_rates']['comparison_gs'], self.data['vacancy_rates']['comparison_ron']))
        
        st.info(self.generate_comparison_comment("affordability", self.data['affordability']['percentage'], 
                                    self.data['affordability']['comparison_gs'], self.data['affordability']['comparison_ron']))
        
        # Time series charts if available
        has_time_series = any(
            self.data.get(metric, {}).get('time_series') 
        ...
        if has_time_series:
            ...
            # Median Rent Tab
            with tabs[0]:
                rent_series = self.data.get("median_rent", {}).get("time_series")
                if rent_series:
                    ...
                    fig = px.line(
                        ...
                        x='date', 
                        y='value', 
                        title=f"Median Weekly Rent for {self.selected_geo_name}",
            ...
            # Vacancy Rates Tab
            with tabs[1]:
                vacancy_series = self.data.get("vacancy_rates", {}).get("time_series")
                if vacancy_series:
                    # Convert to dataframe for plotting
                    ...
                    fig = px.line(
                        ...
                        x='date', 
                        y=y_column, 
                        title=f"Vacancy Rate for {self.selected_geo_name}",
            ...
            # Affordability Tab
            with tabs[2]:
                affordability_series = self.data.get("affordability", {}).get("time_series")
                if affordability_series:
                    ...
                    fig = px.line(
                        ...
                        x='date', 
                        y='value', 
                        title=f"Rental Affordability for {self.selected_geo_name}",
        ...
        excel_data = self.create_excel_output()
        
        st.download_button(
            label="Download Excel Report",
            data=excel_data,
            file_name=f"{self.selected_geo_name}_{self.selected_geo_area}_Rental_Analysis.xlsx",
...
def main():
    ...
    You can either:
    1. Upload individual data files for each category, or
    ...
    with data_source_tab:
        ...
        # Dictionary to store uploaded files by category
        uploaded_files = {
            "median_rents": [],
            "census_dwelling": [],
            "census_demographics": [],
        ...
        with col1:
            ...
            if census_files:
                for file in census_files:
                    ...
                    geo_area = None
                    for area in analyzer.GEO_AREAS:
        ...
        if total_files > 0:
            ...
            # Store the uploaded files in the analyzer
            analyzer.uploaded_files = uploaded_files
            analyzer.files_found = True
    ...
    with folder_tab:
        ...
        if st.button("Scan Folder", key="scan_folder_button"):
            if root_folder:
                ...
                uploaded_files = analyzer.scan_root_folder(root_folder)
                
                # Store the uploaded files in the analyzer
                analyzer.uploaded_files = uploaded_files
            else:
                st.error("Please enter a root folder path.")
    
    # Proceed only if files are found
    if analyzer.files_found:
        st.header("Geographic Selection")
        
        # Get unique geographic areas from all files
        available_geo_areas = set()
        for data_type, files in analyzer.uploaded_files.items():
            for file in files:
        ...
        if not available_geo_areas:
            ...
            # Store the selected geo area
            analyzer.selected_geo_area = selected_geo_area
            
            # Get geographic names for the selected area
            geo_names = analyzer.get_geo_names(selected_geo_area, analyzer.uploaded_files)
            ...
            if geo_names:
                ...
                if st.button("Generate Analysis", type="primary"):
                    with st.spinner(f"Analyzing data for {selected_geo_name}..."):
                        ...
                        analyzer.collect_data(analyzer.uploaded_files)
